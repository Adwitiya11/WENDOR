...
// Blynk Virtual Pins
#define LEVEL_SENSOR_1_PIN V1
#define LEVEL_SENSOR_2_PIN V2
#define LEVEL_SENSOR_3_PIN V3
...
void loop() {
 ...
 // Run DC motor2 to mix coffee and water
  ...
  // Send level sensor values to Blynk
  Blynk.virtualWrite(LEVEL_SENSOR_1_PIN, levelSensor1State);
  if ( levelSensor1State< lowLevel) {
  Serial.println("Level low");
  Blynk.virtualWrite(LEVEL_SENSOR_1_PIN, "Low");
} else if (levelSensor1State < intermediateLevel) {
  Serial.println("Level below intermediate");
  Blynk.virtualWrite(LEVEL_SENSOR_1_PIN, "Below Intermediate");
} else if (levelSensor1State < highLevel) {
  Serial.println("Level above intermediate");
  Blynk.virtualWrite(LEVEL_SENSOR_1_PIN, "Above Intermediate");
} else {
  Serial.println("Level high");
  Blynk.virtualWrite(LEVEL_STATUS_PIN, "High");
}

  Blynk.virtualWrite(LEVEL_SENSOR_2_PIN, levelSensor2State);
   if ( levelSensor2State< lowLevel) {
  Serial.println("Level low");
  Blynk.virtualWrite(LEVEL_SENSOR_2_PIN, "Low");
} else if (levelSensor2State < intermediateLevel) {
  Serial.println("Level below intermediate");
  Blynk.virtualWrite(LEVEL_SENSOR_2_PIN, "Below Intermediate");
} else if (levelSensor2State < highLevel) {
  Serial.println("Level above intermediate");
  Blynk.virtualWrite(LEVEL_SENSOR_2_PIN, "Above Intermediate");
} else {
  Serial.println("Level high");
  Blynk.virtualWrite(LEVEL_STATUS_PIN, "High");
}
  Blynk.virtualWrite(LEVEL_SENSOR_3_PIN, levelSensor3State);
   if ( levelSensor3State< lowLevel) {
  Serial.println("Level low");
  Blynk.virtualWrite(LEVEL_SENSOR_3_PIN, "Low");
} else if (levelSensor3State < intermediateLevel) {
  Serial.println("Level below intermediate");
  Blynk.virtualWrite(LEVEL_SENSOR_3_PIN, "Below Intermediate");
} else if (levelSensor3State < highLevel) {
  Serial.println("Level above intermediate");
  Blynk.virtualWrite(LEVEL_SENSOR_3_PIN, "Above Intermediate");
} else {
  Serial.println("Level high");
  Blynk.virtualWrite(LEVEL_STATUS_PIN, "High");
}
